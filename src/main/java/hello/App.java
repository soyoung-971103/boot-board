...
public class App 
{
    ...
    {
    	//IoC Container : Inversion of Controle (제어의 역전)
    	ApplicationContext ctx = new ClassPathXmlApplicationContext("beans.xml");
  
    	//Greet 인터페이스 선언된 메소드는 sayHello 밖에 없음
    	Greet g = (Greet) ctx.getBean("korea");
    	System.out.println( "Greeting!" + g.sayHello() ); //인터페이스 상속한 클래스에서 추가한 메서드를 사용할 수 없다.
    	System.out.println( "Greeting!" + g.sayHello() + ((Greeter)g).iam1()); //클래스로 형변환을 해줘야 사용가능
